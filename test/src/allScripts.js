				...
				var o = option,
					...
					main = {
						...
						themes: {
							...
							getCls: function( ID ){
								var _t = this, n = ( ID.attr('class') || '' ).split(' '), k = '';
								for( var i = 0; i < n.length; ++i ){
									if( n[ i ].indexOf('theme-KK') != -1 ){
										k = n[ i ];
										break;        
									}
								}
						...
						set: function(){
							...
							else if( ID.hasClass('widget-view-five') ) typ = 'viewFive';
...
var bdy = $('body'),
	...
	pages = {
		...
		buying: {
			el: { wrp: '.page-buying-guide', btn: '.buying-guide-icons [rel]', target: '.prd-info-tab .content-js > li' },
			cls: { selected: 'selected' },
			addEvent: function(){
				var _t = this;
				$( _t.el.btn )
				.unbind('click')
				.bind('click', function(){
					var ths = $( this ), elm = $( _t.el.target + '[rel="'+ ths.attr('rel') +'"]' );
					if( uty.detectEl( elm ) ){
						elm.addClass( _t.cls['selected'] ).siblings().removeClass( _t.cls['selected'] );
						uty.pageScroll({ scrollTop: elm.offset().top });
					}
				})
			},
			init: function(){
